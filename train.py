...
def train(batch_size, epochs, log, output):
    ...
    outputs = tf.transpose(lstm_output, [0,1,2])
    means= tf.transpose(collect_means, [0,1,2])
    locs= tf.transpose(collect_locs, [0,1,2])
    ...
    rewards = tf.expand_dims(tf.cast(correct_prediction, tf.float32), 1)
#     print(rewards.shape)
    ...
    b_t = tf.reduce_mean(rewards, 0) # 1 x T tensor
    
    gradient = tf.reduce_mean(tf.reduce_mean(tf.tensordot(tf.multiply(tf.transpose(outputs, perm=[2, 0, 1]),
                                                                      tf.subtract(rewards, b_t)), 
                                                          tf.subtract(locs, mean),
                                                          axes=[[2], [1]]),
                                             axis=1), 
                              axis=1)
    
    print(gradient.shape)
    vars_location = tf.get_collection(tf.GraphKeys.VARIABLES, scope= "next_loc")
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            ...
            batch_x, batch_lbl = mnist.train.next_batch(batch_size)
